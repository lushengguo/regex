...
class NFA
{
  public:
    ...
    static const NFA *node(ID id) { return nfas.at(id); }
...
{
  public:
    bool regex_match(std::string_view s, const Regex &re) override;
    std::string regex_replace(
      std::string_view s, const Regex &re, std::string_view pattern) override;
    std::unique_ptr<MatchResult> regex_search(std::string_view s, const Regex &re) override;
};
} // namespace regex
